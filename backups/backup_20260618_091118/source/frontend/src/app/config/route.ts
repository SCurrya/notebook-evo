 ...
 *
 * Mobile build note: When BUILD_TARGET=mobile, this route is not used (the mobile app
 * discovers API URL at runtime via api-discovery.ts). We export a static placeholder
 * to satisfy `output: export` requirements.
 */

// Static export compatibility: force-static works for both mobile (export) and web (standalone)
// Web version falls back to NEXT_PUBLIC_API_URL or rewrites when apiUrl is empty
export const dynamic = 'force-static'

export async function GET(request: NextRequest) {
  // Mobile build: return empty placeholder (mobile app uses api-discovery.ts instead)
